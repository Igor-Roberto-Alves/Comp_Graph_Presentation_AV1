...
class SimpleMaterial:
    ...
    def shade(self, hit_record, scene):
        ...
        amb_light_tensor = torch.as_tensor(scene.ambient_light, device=device, dtype=torch.float32)
        diff_color_tensor = torch.as_tensor(self.diff_color, device=device, dtype=torch.float32)
        spec_color_tensor = torch.as_tensor(self.spec_color, device=device, dtype=torch.float32)
        ...
        for light in scene.lights:
            ...
            light_pos_tensor = torch.tensor(light.pos, device=device, dtype=torch.float32)
            light_color_tensor = torch.tensor(light.color, device=device, dtype=torch.float32)
            ...
            diff_contribution = (diff_color_tensor * light_color_tensor) * (self.diff_coeff * diff_intensity)
            ...
            spec_contribution = (spec_color_tensor * light_color_tensor) * (self.spec_coeff * spec_intensity)

            # Adiciona a contribuição desta luz ao total
            total_color += vis*(diff_contribution + spec_contribution) * light.intensity
