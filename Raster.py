...
def main(args):
    ...
    with torch.no_grad():
        final_colors = torch.zeros(
            (camera.img_height * camera.img_width, 3), device=device
        )
        ...
        print(
            f"Renderização concluída com sucesso! Imagem salva como '{output_filename}'"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-s", "--scene", type=str, help="Scene name", default="ball_scene"
    )
    parser.add_argument(
        "-n",
        "--num_samples",
    ...
    parser.add_argument(
        ...
        "--lens_samples",
        type=int,
        help="Number of samples on the lens for depth of field",
        default=1,
    )
    parser.add_argument(
        "-j",
        "--num_jobs",
        type=int,
        help="Number of parallel jobs for rendering",
        default=4,
    )
    parser.add_argument(
        "-o", "--output", type=str, help="Output image file name", default="output.png"
    )
    parser.add_argument(
        "-t",
        "--theta",
