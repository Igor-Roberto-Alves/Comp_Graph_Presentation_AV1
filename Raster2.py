...
def render_colors(scene, ray, hit_record):
    ...
    # Começamos do 1 porque 0 é fundo no hit_mask
    for i, material in enumerate(scene.materials):
        target_id = i + 1
        mat_mask = hit_record.hit_mask == target_id

        if mat_mask.any():
            # Cria um hit_record temporário apenas para os pixels deste material
            # ou garante que o material.shade saiba lidar com a máscara
